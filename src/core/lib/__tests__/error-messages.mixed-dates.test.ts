...
const problem = (code: string, detail: string) =>
  new HttpError({
    ...
    code,
    message: detail,
    problem: { type: "about:blank", title: "x", status: 422, code, detail },
  });
...
describe("errorMessage · dos salidas en un pedido (Cobros premium P2)", () => {
  it("el 422 de fechas mezcladas se dice en español y con qué hacer", () => {
    expect(
      errorMessage(
        problem(
          "orders/mixed_service_dates",
          "Items con fechas de servicio distintas",
        ),
      ),
    ).toBe(
  ...
  it("otro código del slice de pedidos no toma ese texto", () => {
    expect(errorMessage(problem("orders/empty_order", "vacío"))).toBe(

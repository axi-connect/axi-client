...
function Sidebar({
  ...
  return (
    <div
      className="group peer text-foreground hidden lg:block"
      data-state={state}
    ...
    >
      ...
      >
        <div
          ...
          className="glass bg-background group-data-[variant=floating]:border-border flex h-full w-full flex-col group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border"
        >
          {children}
        </div>
      </div>
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  ...
  return (
    <button
      ...
      className={cn(
        "hover:after:bg-[var(--color-border)] absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 transition-all ease-linear group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] sm:flex",
...
function SidebarSeparator({
  ...
  return (
    <Separator
      ...
      className={cn("bg-[var(--color-border)] mx-2 w-auto", className)}
      {...props}
    />
  )
}
...
function SidebarMenuSkeleton({
  ...
  showIcon?: boolean
  /** Ancho del texto simulado. Usar valores deterministas (no aleatorios):
   *  el componente se renderiza en SSR y un ancho random rompe la hidratación. */
  width?: string
}) {
  return (
    <div

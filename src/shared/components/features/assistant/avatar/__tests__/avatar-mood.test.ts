...
const axel = (over: Partial<NonNullable<AssistantMoodInput["lastMessage"]>> = {}) => ({
...
const owner = (over: Partial<NonNullable<AssistantMoodInput["lastMessage"]>> = {}) => ({
  ...axel(over),
  role: "user" as const,
});

describe("resolveAssistantMood", () => {
  it.each<[string, Partial<AssistantMoodInput>, AssistantMood]>([
    ...
    expect(resolveAssistantMood({ ...base, ...over })).toBe(expected);
  });
});

describe("mapas y guardas", () => {
  const ALL: AssistantMood[] = ["idle", "listening", "thinking", "speaking", "proud", "curious", "sorry", "asleep"];

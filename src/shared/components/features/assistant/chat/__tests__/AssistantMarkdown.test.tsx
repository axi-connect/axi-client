...
describe("AssistantMarkdown", () => {
  it("las viñetas son una lista nativa, no párrafos con guiones", () => {
    const { container } = render(<AssistantMarkdown text={"- uno\n- dos"} />);
  ...
  it("la lista numerada arranca donde la escribió el modelo", () => {
    const { container } = render(<AssistantMarkdown text={"3. tres\n4. cuatro"} />);
    expect(container.querySelector("ol")?.getAttribute("start")).toBe("3");
  });

  it("la negrita es <strong> y el código es <code>, con su tipografía", () => {
    const { container } = render(<AssistantMarkdown text="Van **$18.420.000** en `pos_web`" />);
  ...
  it("una etiqueta escrita por el modelo se ve como TEXTO, nunca se ejecuta", () => {
    const { container } = render(
      <AssistantMarkdown text="<img src=x onerror=alert(1)> y <b>negrita</b>" />,
    );
  ...
  it("el salto de línea del modelo se conserva", () => {
    const { container } = render(<AssistantMarkdown text={"Marcela — 92\nDiana — 88"} />);
    expect(container.querySelectorAll("br")).toHaveLength(1);
  });

  it("el cursor solo aparece cuando se está escribiendo, y en el último bloque", () => {
    const { container: quiet } = render(<AssistantMarkdown text="listo" />);
    expect(quiet.querySelectorAll(".animate-pulse")).toHaveLength(0);

    const { container: live } = render(<AssistantMarkdown text={"## Título\nescribiendo"} caret />);
  ...
  it("texto vacío con cursor no revienta: es el primer delta del turno", () => {
    const { container } = render(<AssistantMarkdown text="" caret />);

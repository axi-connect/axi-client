import { parseAssistantText, parseInline } from "../markdown";
...
describe("parseAssistantText", () => {
  it("separa párrafos por línea en blanco y conserva los saltos de dentro", () => {
    ...
    const blocks = parseAssistantText("Marcela — 92\nDiana — 88\n\nY eso es todo.");
  ...
  it("reconoce el título con dos almohadillas, y tolera una o tres", () => {
    for (const prefix of ["#", "##", "###"]) {
      const blocks = parseAssistantText(`${prefix} La fuga son los carritos`);
      expect(blocks[0]).toEqual({
        kind: "heading",
  ...
  it("varias líneas de cita son UNA cita", () => {
    const blocks = parseAssistantText("> Empieza por los carritos.\n> Es la fuga grande.");
    expect(blocks).toHaveLength(1);
    expect(blocks[0]?.kind).toBe("quote");
  });

  it("agrupa las viñetas en una lista y conserva su formato interno", () => {
    const blocks = parseAssistantText("- 30 sin **respuesta**\n- 17 estancadas");
  ...
  it("la lista numerada recuerda su número: una que empieza en 3 no se renumera", () => {
    const blocks = parseAssistantText("3. Marcela\n4. Diana");
  ...
  it("cambiar de tipo de lista abre otra: no se mezclan viñetas con números", () => {
    const blocks = parseAssistantText("- uno\n1. dos");
  ...
  it("un guion sin espacio NO es una viñeta: es un texto que empieza por guion", () => {
    const blocks = parseAssistantText("-30% de cierre");
  ...
  it("una respuesta a medio escribir se parsea sin perder lo que ya llegó", () => {
    ...
    const blocks = parseAssistantText("## La fuga son los carr");
    expect(blocks[0]).toEqual({
      kind: "heading",
      spans: [{ kind: "text", text: "La fuga son los carr" }],
    });
    expect(parseAssistantText("- uno\n- do")).toMatchObject([{ kind: "list" }]);
  });
});

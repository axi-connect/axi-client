...
describe("AgentPreview", () => {
  it("presenta al agente con su nombre, su empresa y su personaje, y pregunta por el producto del nicho", () => {
    render(<AgentPreview name="Joao" tone="cercano" appearance={{ character: "cloudee", color: "sky" }} companyName="La Parrilla" nicheCode="restaurants" />)
    const phone = screen.getByTestId("agent-preview")
    expect(phone).toHaveTextContent("Joao")
    expect(phone).toHaveTextContent("Cloudee · en línea")
    // La cara del teléfono es el personaje elegido, estático
    expect(phone.querySelector('svg[data-character="cloudee"][data-color="sky"]')).not.toBeNull()
  ...
  it("no repite la empresa si el nombre ya la lleva, y sin nombre habla «tu agente»", () => {
    const { rerender } = render(<AgentPreview name="Vendedor de La Parrilla" tone="cercano" appearance={{ character: "nova", color: "white" }} companyName="La Parrilla" nicheCode={null} />)
    expect(screen.getByTestId("agent-preview")).toHaveTextContent(/Soy Vendedor de La Parrilla\./)
    rerender(<AgentPreview name="   " tone="cercano" appearance={{ character: "nova", color: "white" }} companyName={null} nicheCode={null} />)

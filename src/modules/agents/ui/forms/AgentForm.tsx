...
import { listIntention } from "@/modules/agents/infrastructure/services/intention-service.adapter"
import { listCharacters } from "@/modules/agents/infrastructure/services/character-service.adapter"
import { createAgent, updateAgent } from "@/modules/agents/infrastructure/services/agent-service.adapter"
import { listCompanyOptions } from "@/modules/companies/infrastructure/company-service.adapter"
import { agentFormSchema, buildAgentFormFields, defaultAgentFormValues } from "@/modules/agents/ui/forms/config/agent.config"
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          ...
          cache.set("intentions", (intentRes.data?.intentions ?? []).map((i: any) => ({ id: Number(i.id), label: `${i.code} — ${i.flow_name}` })))

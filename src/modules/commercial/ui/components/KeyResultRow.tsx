 ...
 * derecha un solo indicador (la regla fina), el badge si hace falta y el
 * chevron que aparece al pasar el ratón o al enfocar con el teclado.
 *
 * La fila ENTERA es un enlace al detalle (F6): filas clicables como enlaces,
 * no como `div` con `onClick` (M-fila del mockup corregido).
 */
  ...
  return (
    <li className="grouped-row group">
      <Link
        href={href}
        className={cn(
          "grid grid-cols-[minmax(0,1fr)_auto] items-center gap-x-4 gap-y-px px-4 py-3 transition-colors",
          "hover:bg-foreground/[0.03] focus-visible:bg-foreground/[0.03] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring",
        )}
      >
        ...
        {extra !== undefined && extra !== null ? (
          <span className="col-start-1 text-[12.5px] text-muted-foreground">{extra}</span>
        ) : null}
      </Link>
    </li>
  );
...
export function Rule({ pct, soft = false }: { pct: number; soft?: boolean }) {
  ...
  return (
    <span aria-hidden className="block h-1 w-24 shrink-0 overflow-hidden rounded-full bg-secondary">
      <span
        className={cn("block h-full rounded-full", soft ? "bg-muted-foreground/50" : "bg-brand")}

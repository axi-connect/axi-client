...
const mockMutate = jest.fn<
  Promise<unknown>,
  [{ code: string; forced: "on" | "off" | null; reason?: string }]
>();
jest.mock(
  "@/modules/platform/infrastructure/api/hooks/use-tenant-features",
  () => ({
...
describe("TenantFeaturesView (premium P1: quién decide cada función)", () => {
  ...
  it("la isla cuenta solo las fijadas por la plataforma y lleva a Auditoría", () => {
    ...
    const island = screen.getByRole("region", {
      name: "Fijadas por la plataforma",
    });
    expect(island).toHaveTextContent("1de 3 funciones");
    expect(island).toHaveTextContent(
      "Documentos del cliente · forzada apagada",
    );
    expect(
      within(island).getByRole("link", { name: "Ver en Auditoría" }),
    ).toHaveAttribute("href", "/platform/tenants/t1/audit");
  });

  it("la escalera marca SOLO el peldaño que manda, con su valor", () => {
    render(<TenantFeaturesView tenantId="t1" />);
    const fx = screen.getByRole("list", {
      name: "Quién decide Precios en otra moneda",
    });
  ...
  it("forzar exige motivo; un motivo rápido lo escribe y se envía con el override", async () => {
    ...
    fireEvent.click(
      within(
        screen.getByRole("radiogroup", { name: "Override de Planes de pago" }),
      ).getByRole("radio", { name: /Forzar OFF/ }),
    );

...
const pay = (id: string, status: OrderPaymentDTO["status"], amount: number | null, created_at: string): OrderPaymentDTO =>
  ({ id, status, amount_cents: amount, currency: "COP", method_label: "Nequi", created_at, attachment_id: null }) as unknown as OrderPaymentDTO;

const order = (payments: OrderPaymentDTO[]): OrderDTO =>
  ({ id: "o1", currency: "COP", balance_cents: 19_458_586_00, payments }) as unknown as OrderDTO;
...
describe("PendingProofIsland (premium P3: lo más accionable del pedido)", () => {
  it("sin comprobantes por revisar no existe: no se inventa otro «lo próximo»", () => {
    const { container } = render(
      <PendingProofIsland order={order([pay("a", "verified", 8_339_394_00, "2026-09-22T10:00:00Z")])} onReview={jest.fn()} />,
    );
  ...
  it("con dos pendientes cuenta los dos, muestra el MÁS ANTIGUO y lo que faltaría si es cierto", () => {
    ...
    render(
      <PendingProofIsland
        order={order([pay("new", "reported", 1_000_000_00, "2026-09-25T15:00:00Z"), older])}
        onReview={onReview}
      />,
    );
    const island = screen.getByRole("region", { name: "Comprobante por revisar" });
  ...
  it("un comprobante sin monto no calcula un saldo que no se puede saber", () => {
    render(<PendingProofIsland order={order([pay("x", "reported", null, "2026-09-25T09:41:00Z")])} onReview={jest.fn()} />);

...
jest.mock("@/modules/orders/infrastructure/services/order-payments-service.adapter", () => ({
  getPaymentProofUrl: (orderId: string, paymentId: string) => mockProof(orderId, paymentId),
}));
...
const payment = (attachment_id: string | null): OrderPaymentDTO =>
  ({ id: "pay-1", attachment_id, mime_type: "image/png" }) as unknown as OrderPaymentDTO;

describe("PaymentProofViewer (QA R6: el 422 repetido de GET /attachment)", () => {
  beforeEach(() => mockProof.mockReset().mockResolvedValue({ url: "https://s3.test/proof.png" }));

  it("un pago SIN comprobante no pide la URL firmada ni pinta nada", () => {
    const { container } = render(<PaymentProofViewer orderId="o1" payment={payment(null)} />);
  ...
  it("un pago CON comprobante sí la pide y muestra la imagen", async () => {
    ...
    await waitFor(() => expect(screen.getByAltText("Comprobante de pago")).toHaveAttribute("src", "https://s3.test/proof.png"));
  });
});

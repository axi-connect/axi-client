...
**Nav público.** El header (`SiteHeader.tsx`) quedó saneado en F2 del GTM (rutas reales, a11y, `glass-overlay`), pero es un menú de dos desplegables de 320px con cinco enlaces de texto. El producto tiene once áreas de capacidad, cuatro canales, tres pilotos reales con 185 productos y un diferenciador de adopción que nadie más tiene (conectar el número actual y vender el mismo día) — y nada de eso se ve en el menú. Además el modelo es **alta asistida, sin auto-registro**: el único acto de conversión es agendar la demo o escribir por WhatsApp, así que cada superficie del nav tiene que empujar ahí. Hoy tres de los cinco ítems del nav llevan a un andamio (`PageOutline`, marcado como provisional) y "Precios" es un ancla dentro de una landing de doce secciones.

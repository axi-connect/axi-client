...
| F1 | **Construida 2026-09-24, pendiente de certificación y de visual del dueño** | Backend: `QualityRunKind.interactive` (+ migración), `TenantEligibilityService` compartido con `start_run`, `EnsureSimulatorChannelUseCase.ensureExists` (no re-apunta), `CreateSession`/`SendSessionMessage`/`EndSession`, `SessionsQuery` (`?after=` delta, `agent_changed`), traza por Redis (`AgentTraceService` sink cuando `simulated`), `RedisSimTraceReaderAdapter`, sweep `session_sweep` cada 5 min, `GET /platform/tenants/:id/agents`, gasto diario `quality:sim:daily_spend:*`, advisory locks global+tenant en el create. **Hallazgos al construir:** (a) `pg_advisory_xact_lock` devuelve `void` y `$queryRaw` no lo deserializa → `::text` como en `ensure_default_pipeline`; (b) las clases DTO llevan prefijo `Quality…` porque `intake` ya expone `CreateSessionDto`/`SessionDetailDto` y swagger nombra los schemas por la clase (la colisión pisaba el contrato de las entrevistas en silencio); (c) el `schema.d.ts` del cliente se EMPALMA (no se regenera entero): el committeado va por delante de server main (memoria `contrato-cliente-adelantado-a-server-main`). Frontend: tab «Simulacro», `SimulatorView` de tres columnas, `ChatBubble` compartido (lo usa también `TranscriptPanel`), `AgentSelect` (el wizard `TargetStep` migra a él), hooks con polling adaptativo y envío optimista. Fuera de F1 a propósito: composer de medios (F2), «convertir en escenario» (F5). Verjas corridas por el constructor: server `typecheck` + eslint de los 47 archivos tocados + 7 suites unitarias afectadas (57 tests) + e2e `interactive_session` 10/10 (el e2e completo pasó salvo `fx_rates`, dependencia externa preexistente) + `openapi:generate` (diff = 6 rutas, 7 schemas nuevos, `RunsPageDto`/`RunDetailDto` con `kind` ampliado); cliente `tsc` (solo el error preexistente de `ConversationPanel.test.tsx`), eslint de los 27 archivos tocados, 4 suites (31 tests). Suite unitaria completa del servidor: 622/622 suites. **Correcciones de la auditoría de F1 (axi-f2, 2026-09-24):** B1 el id optimista `pending-…` viajaba como cursor `after` y el server lo rechazaba (400 en cada poll: el chat se congelaba tras el primer envío) → `lastMessageId` salta las burbujas optimistas y hay test del poll posterior al envío; G1 el estado del agente y `agent_changed` se derivaban solo del delta → ahora se calculan sobre toda la conversación (último mensaje, último turno del agente, `count` de turnos de otro agente, `distinct` de emisores) y el cursor es `(created_at, id)` (N2, relojes desfasados entre pods); M1 el trace ya no serializa el turno real cuando el tracing está apagado; M2 `EndSession` idempotente en carrera (el contador del run sube solo si esta llamada asentó el case); M3 `SendSessionMessage` actualiza `params` con `jsonb` atómico y solo si sigue `running`; M4 sin código muerto (`isSessionActive`, `QualitySessionSummaryDto`, cap key en el MGET, literal duplicado); M5 `AgentSelect` en `quality/shared`; N1 `ensureExists` sin lookup duplicado. Pendiente: `next build` y recorrido real (los corre la auditora) y visual del dueño. |
| F2 | **Construida 2026-09-24, pendiente de certificación** | Medios en el simulacro. Backend: `sim-media:{mime}:{key}` (codec en `simulator_channel.port`, prefijos acotados a `companies/{cid}/quality/sim/` y `quality/datasets/`; el adapter exige además que el `cid` sea el del tenant del job), `downloadMedia` desde storage, rama sin credenciales en `media_ingest` solo para `simulator`, `simulated` en los jobs de reconocimiento/STT (el canal se carga antes del early-return del retry), `bill_to platform` + sin cuota en visión/embeddings/`recordRecognition`/STT (`VisualSearchInput.bill_to` aditivo), `POST /sessions/:id/media` (multer 5 MB aborta el stream; mime por BYTES con `sniffMediaMime`; notas de voz webm/mp4 → ogg/opus por `AUDIO_TRANSCODE`, que `ConversationsModule` ahora exporta), staging `companies/{cid}/quality/sim/{runId}/` registrada en `params.media_keys` (jsonb atómico) y borrada al terminar, adjuntos presignados 5 min + `recognition`/`transcription` proyectados en el transcript. M6 de la auditoría (company_id en el SQL crudo) cerrado. e2e: imagen por la ingesta real con `skipped/disabled`, storage_bytes a plataforma, 422 para PDF, y (O2) con reconocimiento encendido la visión llega con `bill_to platform`, `product_recognitions` y `ai_tokens_input` con `resource_id = conversation` y el gasto de la sesión sube; staging borrada tras finalizar. Frontend: `Composer` con imagen (≤5 MB, caption = texto escrito), nota de voz (grabador del inbox, preview y enviar/descartar), ubicación (diálogo con presets), `useSendSessionMedia` (multipart con Bearer, burbuja optimista reconciliada por provider_message_id), `MediaBubble`/`RecognitionChip`/`TranscriptionLine`. |
| F3–F5 | Sin empezar (autorizadas por el dueño sin gate: certificación → siguiente) | |

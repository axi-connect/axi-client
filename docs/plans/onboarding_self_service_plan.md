...
> **Estado (2026-09-02):** F0-A y F0-B aprobados. **F1–F6 implementadas y commiteadas** en `feat/onboarding-self-service`, sin push: precios, `/comenzar`, shell de `/onboarding`, nicho, horarios, catálogo con IA, agentes desde plantilla, WhatsApp embebido (`ConnectChannelFlow` extraído de `ConnectChannelView` y publicado por `channels/public.ts`), «Listo» con entitlements y `TrialCountdownBanner` → `/billing`. Sigue **F7** (docs, `api:types` cuando el backend esté en main, sustituir DTO manuales por `Schemas[...]`, reindexar grafo). Nota: los nichos son catálogo cerrado en `domain/niches.ts`; los entitlements viven en `onboarding/domain/entitlements.ts` (su único consumidor hoy); el sheet de plantilla expone nombre, tono, personalidad y datos clave.
>

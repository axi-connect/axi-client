...
> **Estado (2026-09-02):** F0-A y F0-B aprobados. **F1–F7 implementadas y commiteadas** en `feat/onboarding-self-service`, sin push. F7 cerró la documentación (`docs/modules/onboarding.md`, `architecture.md` §1/§3.3/§6/§13, `public-site.md` §3/§4.2/§4.3) y verificó que `schema.d.ts` no tiene drift. **Queda bloqueado por el backend:** hasta que B1–B4 estén en `main` de axi-server no se puede correr `npm run api:types` ni sustituir los tipos `// CONTRACT` por `Schemas[...]` (lista en `docs/modules/onboarding.md` Parte C); el reindexado del grafo va tras la fusión. Orden de fusión: backend primero, luego este cliente (`api:types:check`).
>

...
> **Estado (2026-09-15, tarde): F1 + F2 + F4 IMPLEMENTADAS en el worktree `cmo-axel-avatar`.** Verja acotada verde: `tsc` sin errores nuevos (queda uno preexistente en `inbox/ConversationPanel.test.tsx`, ajeno), 19 suites / 201 tests del módulo CMO y `core/styles`, lint limpio. **La F0.5 (spike con el Studio de Avatar Lab) quedó sin objeto**: la cara se autoró a mano en nuestro propio esquema durante el mockup y el dueño la aprobó, así que el Studio, el submódulo en `axi-video` y el conversor NO se crean; la sección 2 queda como historia de la decisión de licencia. Pendiente del dueño: verificación visual en `/cmo` y `/cmo/settings` (claro/oscuro, táctil, reduced-motion), elegir diadema o nada (interruptor en Apariencia), y la fusión a `main`. Después: la simplificación del despacho (menos texto, más minimalista), que el dueño pidió dejar para después del avatar.
...
| ~~**F0.5 Spike go/no-go**~~ **SIN OBJETO** (la cara se autoró a mano en el mockup; no hace falta el Studio) | Studio instalado en `axi-video/tools/avatar-lab`; Axel autorado y exportado; `convert.mjs` mínimo; `AxelAvatar lg` con rig + botones de expresión al lado del raster actual; 50 `sm` por sprite + 1 `lg` con deltas simulados; Safari/Firefox (`<use>` + tokens, fallback de `linear()`); `next build` delta; test jest de presupuesto. | **Go** si: el dueño prefiere el SVG al raster; idle ≈ 0 %; paint ≤ 0.5 ms/frame; 0 re-renders en 50 deltas; sprite temático correcto en 3 navegadores; bundle ≤ 6 KB. **No-go parcial**: SVG solo en el hero, raster/`Sparkles` en `sm`. **No-go total**: dos frames raster con crossfade. |
| **F1 Dominio + renderer — HECHA** | `axel-avatar.ts`, `axel-mood.ts`, JSON + data, `AxelAvatar`/`AxelAvatarSprite`, `cssEase`, bloque CSS; tests de dominio y de presupuesto. | Verja acotada verde (`npm test -- --maxWorkers=2` sobre `src/modules/cmo`, `tsc`, lint de lo tocado). |
| **F2 Hero — HECHA** | `use-axel-mood`, `use-axel-life`, `AxelHeroAvatar` (escenario + sombra), borrado de `AxelOrb`, `BriefingHero`/`AxelChat` (`ownerTyping`), `CmoBlockedState`. | Test de 0 re-renders verde; visual claro/oscuro; `AxelChat.test` verde. |
...
| **F4 Mirada + gesto — HECHA** (`TAP_WINDOW_MS` subió a 2,5 s: con 1,5 s dos guiños con cooldown ya la agotaban y el saludo era inalcanzable; lo descubrió el test) | `use-axel-gaze`, botón `greet`, easter egg, cooldown. | Tests de hook/componente; prueba manual táctil (sin mirada, toque = guiño) y reduced-motion (imagen, nada se mueve). |
| **F5 Pulido, docs, medición — PARCIAL** (reduced-motion, §6 del DESIGN-SYSTEM, `cmo-orb.webp` borrado y presupuesto ejecutable hechos; falta el protocolo de medición en navegador antes/después, que corre la auditora con `next build`) | Reduced-motion final, §6 del DESIGN-SYSTEM, borrar `cmo-orb.webp`, tabla de medición antes/después rellenada en el plan, README de autoría en `axi-video`. | Checklist §11 del DESIGN-SYSTEM; suites completas y build delegadas a la sesión auditora; el dueño cierra y decide el push. |

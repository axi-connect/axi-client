...
- **Los nodos `Route` NO son los endpoints del backend.** Los 380 nodos mezclan rutas del App Router, navegaciones y literales sueltos de documentación. El path vive en la propiedad **`name`**, no en `path` (`key_path` está vacío salvo nodos de infra).
- **Las llamadas reales al API son aristas `HTTP_CALLS`** (417). Filtra `callee STARTS WITH 'http.'` para quedarte con las **328 reales**; el resto son `router.push`/`router.replace`, o sea navegación de Next, no HTTP.
...
- Hotspots de fan-in de este repo: `cn` (374), `errorMessage` (220), `showAlert` (146), `useAlert` (105), `HttpClient.post`/`get` (50/40). Tocarlos tiene alcance amplio.

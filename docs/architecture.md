...
5. Un slice importa de otro **solo a través de su barrel `public.ts`** (`src/modules/<slice>/public.ts`), que declara explícitamente su superficie pública: tipos de `domain`, labels/helpers puros, sus hooks/store/context y sus puertos. **Prohibido** importar rutas internas de otro slice (`infrastructure/`, `ui/components/`, casos de uso), incluso si el símbolo está exportado. Si un slice necesita algo que no está en el `public.ts` del otro, se **añade al barrel en el mismo PR** (decisión consciente y revisable), nunca se importa por la ruta profunda.
   ...
   - Barrels existentes:
     - `modules/crm/public.ts` — consumido por `inbox` (rail de contexto de la conversación y `getTenantUsers` para el filtro «Asignada a»), `dashboard` (labels de ciclo de vida) y `marketing` (el DSL de audiencia: `AudienceFilterBuilder`, `describeSegmentFilters`, `compactSegmentFilters`, `listSegments`, `listTags`).
     ...
     - `modules/commercial/public.ts` — `GoalProgressBlock` (la franja de la meta del mes, autosuficiente: carga lo suyo del store del slice y no pinta nada sin capacidad, permiso o meta); consumido por `dashboard` (F3 del método comercial, `axi-server/docs/plans/commercial_method_plan.md`). Los tipos y badges del ritmo se publicarán cuando `cmo` los necesite (chip de meta del briefing, F7). `commercial` NO importa de `crm`: enlaza por href.

...
- **Los nodos `Route` NO son los endpoints del backend.** Los 555 nodos mezclan rutas del App Router, navegaciones y literales sueltos de documentación. El path vive en la propiedad **`name`**, no en `path` (`key_path` está vacío salvo nodos de infra).
- **Las llamadas reales al API son aristas `HTTP_CALLS`** (545). Filtra `callee STARTS WITH 'http.'` para quedarte con las **448 reales**; el resto son `router.push`/`router.replace`, o sea navegación de Next, no HTTP.
...
- Hotspots de fan-in de este repo: `cn` (437), `errorMessage` (306), `useAlert` (143), `formatMoney` (74), `useAuth` (64), `HttpClient.post` (57), `isHttpError` (53). Tocarlos tiene alcance amplio.
- **El ranking de `hotspots` también arrastra colisiones con los globals de los tests.** Al crecer la suite, funciones propias que se llaman igual que un global de Jest suben al top con fan-in falso: `channel-health.describe` (121 "callers", todos archivos `__tests__` que en realidad invocan el `describe()` de Jest — 57 → 100 → 106 → 121 en cuatro indexados, así que el artefacto crece con la suite) y `DatabaseConnectionSheet.render` (81, el `render` de Testing Library). Antes de tratar un hotspot como punto caliente real, mira de dónde vienen sus callers.
...
5. Un slice importa de otro **solo a través de su barrel `public.ts`** (`src/modules/<slice>/public.ts`), que declara explícitamente su superficie pública: tipos de `domain`, labels/helpers puros, sus hooks/store/context y sus puertos. **Prohibido** importar rutas internas de otro slice (`infrastructure/`, `ui/components/`, casos de uso), incluso si el símbolo está exportado. Si un slice necesita algo que no está en el `public.ts` del otro, se **añade al barrel en el mismo PR** (decisión consciente y revisable), nunca se importa por la ruta profunda.
   ...
   - Barrels existentes:
     - `modules/crm/public.ts` — consumido por `inbox` (rail de contexto de la conversación y `getTenantUsers` para el filtro «Asignada a»), `dashboard` (labels de ciclo de vida) y `marketing` (el DSL de audiencia: `AudienceFilterBuilder`, `describeSegmentFilters`, `compactSegmentFilters`, `listSegments`, `listTags`) y `analytics` (`STAGE_KIND_LABELS`/`STAGE_KIND_ORDER`: nombrar y ordenar el «Recorrido del pipeline» de Conversión, F7 del método comercial).
     - `modules/catalog/public.ts` — consumido por `marketing` para el `VariantPicker` del regalo de una promoción.
     - `modules/channels/public.ts` — consumido por `marketing` para elegir el canal cloud del que cuelgan las plantillas de Meta (se creó en vez de duplicar la llamada a `/channels`: el dueño del recurso es uno solo), por `onboarding`, que embebe el wizard de conexión (`ConnectChannelFlow`) en su paso «WhatsApp»: es el MISMO flujo de `/settings/channels/connect`, no una copia, y por `inbox` y `workspace` para `ChannelKindIcon` (logo por `kind`) y `listChannels` (opciones del filtro por canal).
     - `modules/companies/public.ts` — `SchedulesEditor`, `loadMyCompanyOnce`, `invalidateMyCompanyCache`, `useMyCompany`; consumido por `scheduling`, `onboarding` y `crm` (el `niche_code` de la empresa pone su plantilla primero en `/crm/settings/recorrido`).
     - `modules/agents/public.ts` — tipos y caché de agentes (para `crm`) y, para `onboarding`, el **perfil del agente** del estudio de agentes (2026-09-21): tipos y constantes del catálogo en código (`AGENT_CHARACTERS`, `AGENT_COLORS`, labels, `DEFAULT_APPEARANCE`, `BRIEF_TONES`), los selectores presentacionales `CharacterPicker`/`ColorPalette`/`VoiceSelector` y `listAiVoices`. Ya no hay characters: el personaje es un código, no una entidad del tenant.
     ...
     - `modules/onboarding/public.ts` — `useOnboardingResume` (autosuficiente: la configuración pendiente como dato, que el `dashboard` pinta en su isla); `NICHES`/`nicheByCode` para `companies` y para `crm` (nombre legible de las plantillas del recorrido, con caída al `name` que manda el servidor si el código no está en el catálogo del cliente).
     - **Previstos por el programa «Cobros, documentos y TRM»** (`docs/plans/cobros_frontend_plan.md`, aprobado 2026-09-16): `modules/orders/public.ts` (para `crm`, `inbox` y `collections`), `modules/collections/public.ts` (`PaymentPlanBlock`, `PaymentProgressChip`, `RegisterInstallmentPaymentDialog`) y `modules/documents/public.ts` (`DocumentsList`, `SendDocumentDialog`); `onboarding/public.ts` ganará `NICHES`/`nicheByCode` para el selector de tipo de negocio de `companies`. Cada barrel aparece en la fase que trae su primer consumidor.
     - `modules/forms/public.ts` — `listForms` y los helpers puros de la definición (`FLOW_LABELS`, `FORM_FLOWS`, `CONTACT_COLUMN_CODES`, `fieldStorageHint`, tipos `FormDefinitionDTO`/`FormFieldDTO`/`FormFlow`); consumido por `crm` para la columna «Datos» de la tabla de contactos y el panel «Datos del cliente». La escritura (`upsertForm`/`deleteForm`) no se publica: el único dueño de la definición es `/settings/forms`.
     - `modules/dashboard/public.ts` — `DashboardCard` y `MetricTile`, la superficie de tarjeta y el tile de métrica del Panel; consumido por `analytics` (antes por ruta profunda; corregido en F3 del método comercial).
     - `modules/commercial/public.ts` — `GoalProgressBlock` (la franja de la meta del mes, autosuficiente: carga lo suyo del store del slice y no pinta nada sin capacidad, permiso o meta), consumido por `dashboard` (F3 del método comercial, `axi-server/docs/plans/commercial_method_plan.md`); `isCommercialProposal`/`commercialProposalHref` y el hook autosuficiente `useGoalChip` («Meta · N %», `null` sin meta, permiso o capacidad), consumidos por `cmo` (la tarjeta de propuesta enlaza las del método a `/comercial/acciones/:id` y el briefing pinta el chip; F6/F7); `useRouteRates` (autosuficiente: qué tasas de cierre usa la ruta según `plan.inputs`, `null` sin plan), consumido por `analytics` para marcar «la tasa que usa la ruta» en «Tasas vivas». `commercial` NO importa de `crm` ni de `cmo` (cmo ya consume `commercial/public`: el ciclo se evita; `OUTREACH_TYPE_LABELS` es un espejo documentado): enlaza por href.
...
- **`providers/`** — `ThemeProvider` (next-themes), `AuthProvider` (sesión + `hasPermission(code)` con wildcard `resource:*`; expone `useAuth`/`useSession`), `AlertProvider` (`showAlert`/`showModal`/`closeModal`, expone `useAlert`; `showAlert` delega en `core/notifications`), `SplashProvider` (splash post-login; expone `useSplash`/`useSplashOptional` + `AppReadySignal`).
- **`notifications/`** — los avisos flotantes sobre sileo: `notify` (única importación de `sileo`, vetada fuera de aquí por ESLint), `to-options.ts` (traducción `showAlert` → sileo: título, duración, ranura) y `NotificationsToaster` (lo monta `AlertProvider`). Ver DESIGN-SYSTEM §9.4.
...
- **`(private)`** — shell autenticado (`SidebarProvider` + `AppSidebar` + `PrivateHeader` + superficie full-width). Dentro, el sub-grupo **`(content)`** centra el contenido (`mx-auto max-w-7xl p-4 md:p-6`, DESIGN-SYSTEM §4.2) para `/dashboard`, `/admin/agents`, `/settings/{company,users,roles}` y `/comercial` (la ruta del mes; `/comercial/meta` fija la meta; el slot `@sheet` pinta `/comercial/resultados/[key]` y `/comercial/acciones/[id]` como rutas interceptadas con `DetailSheet` en navegación suave DENTRO de `/comercial`, y un `@sheet/[...catchAll]` que no pinta nada para que el panel se cierre al navegar a otra ruta del segmento (`/comercial/meta`); las páginas duras montan la ruta del mes detrás del panel y son a las que se llega desde `/cmo` —la intercepción no aplica fuera del segmento—; `comercial/default.tsx` es el respaldo del slot `children`); `/workspace/{inbox,inbox/[id]}` queda fuera del grupo y es full-bleed.
...
- `@sheet` — detalle como panel lateral interceptado: `cmo` (propuesta), `orders`, `scheduling/calendar`, `crm/pipeline`, `billing/invoices` y `comercial` (resultado clave y acción propuesta).
- `@form` — en `rbac` (crear/editar en modal). `admin/agents` dejó de usarlo: crear y editar son páginas completas (`/admin/agents/new`, `/admin/agents/[id]`, el estudio de agentes) porque la pantalla necesita el ancho entero; las URLs viejas redirigen en `next.config.ts`.
- Cada slot lleva `default.tsx → return null` para no renderizar nada cuando está inactivo.
- **Un slot con rutas hermanas en el segmento necesita `[...catchAll]/page.tsx → return null`**: en navegación SUAVE Next conserva el último estado de un slot que no casa con la URL nueva, así que un panel abierto seguía encima al ir a otra página del mismo segmento (C1 de `comercial`: «Corregir» → `/comercial/meta`). La interceptada gana al catch-all por especificidad.

**Rutas interceptadas** `(.)segment` — muestran una ruta destino como overlay/modal en navegación *soft*, y como página completa en navegación *hard*/refresh. Ejemplo: `workspace/@modal/(.)channels/create` renderiza `ChannelForm` en `Modal`.
...
- **Integración por slice** — `channels/infrastructure/hooks/use-channels-realtime.ts` (namespace `/channels` → `channels.store`); `inbox/infrastructure/realtime/use-inbox-socket.ts` (namespace `/inbox` → `inbox.store`, comandos `claim/takeover/return_to_ai/close/send_message/mark_read/typing`, re-join automático al reconectar, `handoff_conflict` → re-fetch). Ese mismo hook escucha además los eventos `contact.*` (incluido `contact.updated`, que emite el backend cuando cambia un dato de la ficha: edición, `save_contact_data` de la IA, revisión del operador), `crm.*` y `order.*` que traen `contact_id` para refrescar el rail de contexto de la conversación (`docs/modules/inbox.md` §C.4).

- **Método comercial (F8)** — `commercial/infrastructure/realtime/use-commercial-realtime.ts` (namespace `/inbox`, room company): `commercial.goal_set` → `refresh()` del store (meta + plan + ritmo; si había una carga en vuelo la espera y vuelve a pedir), `commercial.{plan_recomputed,pace_updated,pace_behind,pace_recovered}` → `reloadPace()`, y en `/comercial` `pace_behind` + `cmo.proposal_{created,decided}` → «Axi propone». Debounce de 1 s por familia; los números de secuencia del store descartan la respuesta vieja. Lo montan `CommercialView` (con propuestas) y `GoalProgressBlock` (la franja del Panel). Al RECONECTAR recarga todo (la primera conexión no). `crm/infrastructure/realtime/use-journey-realtime.ts`, montado una vez en el 360 del contacto, reemite `crm.deal_stage_changed`/`crm.deal_stage_reverted` de ESE contacto como el CustomEvent `crm:journey:changed` (§9) que ya escuchan la card «Recorrido», el historial y la página (`crm.deal_stage_reverted` está tipado pero el servidor aún no lo retransmite por WS).
...
- **`Timeline`** (`timeline/`) — timeline vertical genérico (línea conectora + nodo tonal con icono + hasta 3 líneas). `TimelineItem` = `{ id, icon, tone, title, description?, meta?, badge?, action? }`; `tone` ∈ `neutral|info|success|warning|destructive|violet`; `action` es un nodo a la derecha de la entrada (p. ej. «Deshacer» del recorrido del CRM) que va en un `div.hover-reveal` dentro del `li.reveal-group`: con ratón aparece al hover o al foco, en táctil siempre. Presentacional puro: los datos, la paginación y los labels los aporta el slice. Incluye `TimelineSkeleton` y `AiBadge` (✦IA violeta). **Única implementación del patrón**: la consumen el historial 360 del contacto, la actividad del pedido y el rail de contexto del inbox.
- **`assistant/`** — el **kit de asistente conversacional** (2026-09-18): `AssistantChatShell` (raíz con `data-empty`/`data-docked`, scroller, barra sticky, FLIP del compositor, auto-scroll), `AssistantDock`, `AssistantHeroAvatar` + `AssistantAvatar`/`AssistantStage` (el rig SVG de la cara; `avatar-rig.ts`, `avatar-mood.ts` y `avatar-characters.ts` son TypeScript puro — este último trae la geometría de los cuatro personajes: `lumo` (Axel/Alba, bit a bit la de siempre) y los tres de plataforma `cloudee`/`nova`/`strobi`, más la paleta de 8 códigos; `AssistantAvatar` recibe `character` y `color`, y sin `color` Axel y Alba no cambian un píxel), `AssistantBubble`/`UserBubble`/`SystemNote`, `AssistantQuestion` (lista agrupada iOS, solo la última viva), `AssistantThinking` (pasos, frases o puntos), `AssistantMarkdown`, `AssistantComposer` (autosize, Enter, `voice` opcional) y `StarterPills`. **Presentacional puro**: no importa de `modules/`; cada slice aporta su store, su copy y su personaje por props (`cmo` → Axel con `AxelHeroAvatar`/`useAxelMood`; `intake` → Alba con `AlbaHeroAvatar`/`useAlbaMood`, diadema fija). Sus tipos son estructurales (`AssistantQuestionData` coincide con los DTO de ambos slices). El campo de fondo es `.assistant-field` en el `<main>` de cada vista. Plan: `docs/plans/assistant_kit_premium_plan.md`.

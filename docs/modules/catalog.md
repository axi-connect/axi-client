...
- **Metadatos con IA (plan `catalog_enrichment`)**: sección «Búsqueda con IA» en el detalle (`ProductEnrichmentSection`): la descripción que ve el agente (≤160, reemplaza a la de la ficha), atributos por vertical (etiquetas del backend) y hasta 12 términos como chips; se edita (marca `edited_by_user_at`: el automático no la pisa), se regenera (202 + polling 3 s / 30 s) y se desactiva por producto. Es la ÚNICA superficie violeta de la página (acento de IA). Se edita también en un espejado de Shopify — lo generado es de axi —; solo «Aplicar categoría» respeta `locked_fields` (`locked_category` lo sirve el backend). Adapter: `product-enrichment-service.adapter.ts`. El interruptor «Enriquecer automáticamente» y el selector «Tipo de catálogo» viven en Ajustes › Reconocimiento de producto (slice `agents`, tarjeta «Metadatos con IA» con cifras, «Enriquecer catálogo» y aviso del tope mensual de plataforma). La nota bajo «Con metadatos» (`enrichmentPendingNote`) distingue «en espera por el límite del proveedor · se reanudan solos» (`pending_rate_limited`), «esperan al tope del mes» (`pending_cap`) y «pendientes · en curso», y la tarjeta muestra el consumo del mes: tras el incidente 2026-09-09 una cifra sola de «pendientes» no decía si algo corría.
